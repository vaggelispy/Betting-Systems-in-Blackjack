...
def winningplay(gains,i,bet,gains2,earnings,minn,j,betlimit,winnings,games,mins,g):
    gains += i*bet
    gains2 += i*bet    
    earnings += i*bet
    if i > 0:
...
for winnings in limits:
    ...
    for i in bjgenerate(total_games):
        ...
        g += 1
        gains11 += i*bet1
        bet1,earnings1,gains1 = bs.oscar(i,bet1,earnings1,gains1)
        if earnings1 < min1:
            min1 = earnings1
        if bet1 > betlimit:
            bet1 ,gains1 = 1 , 0      
        ...
        gains2,bet2,gains22,earnings2,min2,games2,mins2 = winningplay(gains2,i,bet2,gains22,earnings2,min2,j2,betlimit,winnings,games2,mins2,g)
        gains3,bet3,gains33,earnings3,min3,games3,mins3 = winningplay(gains3,i,bet3,gains33,earnings3,min3,j3,betlimit,winnings,games3,mins3,g)
        gains4,bet4,gains44,earnings4,min4,games4,mins4 = winningplay(gains4,i,bet4,gains44,earnings4,min4,j4,betlimit,winnings,games4,mins4,g)
    ...
    if len(games4) == 0:
        games4 = [0]
        mins4 = [0]
    print(f'Winnings = {winnings}\nOscar without loss limit : mean of games = {round(mean(games1),2)} and mean of min values = {round(mean(mins1),2)}\nOscar with loss limit 15: mean of games = {round(mean(games2),2)} and mean of min values = {round(mean(mins2),2)}\nOscar with loss limit 20: mean of games = {round(mean(games3),2)} and mean of min values = {round(mean(mins3),2)}\nOscar with loss limit 31: mean of games = {round(mean(games4),2)} and mean of min values = {round(mean(mins4),2)}\n')
